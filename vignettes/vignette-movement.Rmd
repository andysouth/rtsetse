...
Now what happens if we apply this movement to the representation of a real landscape ? Lets first use the Serengeti map dataset stored in rtsetse.
...
Then we convert the vegetation categories to movement multipliers and run the simulation. So the result map below is just for a single age and sex class with movement only and no births and deaths.
...
dfMortByVeg <-  data.frame(code=c("D","T","O","S","B","G","N"),mortality=c(1.5,1.2,0.9,0.8,0.7,0.6,0.5))
