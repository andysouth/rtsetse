                           ...
                           ) 
{
  ...
  #aGrid[x,y,sex,age]
  allArgs <- c(x,y,sex,age)
  
  if ( class(aGrid)!="array" | length(dim(aGrid)) != 4 )
       stop("the first arg needs to be an array with 4 dimensions [x,y,sex,age], yours is a ",class(aGrid)," with length(dim)=",length(dim(aGrid)),"\n")
  #I may also want to test that 
  #if (identical( names(dimnames(aGrid)),c("day","x","y","sex","age")))
  ...
  { 
    ...
    toReturn <- sum( aGrid[x,y,sex,age] ) 
...
#     if ( identical(sex,'all')) marginArg <- c(marginArg,'sex') 
    ...
    toReturn <- apply( aGrid[x,y,sex,age], MARGIN=marginArg, sum )
  }
